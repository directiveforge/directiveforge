...
**Last updated:** 2026-07-10 — **2026-07-10 hardening (Sonnet 5 session, Fable-authored dispatch):** pre-HN truth-debt sweep + model-state refresh + upgrade-plan rename shipped as errata to v0.20.0, staged for push as public commit #2. Provenance closes on push — see `launch/RELEASES.md` for the frozen row once W7 completes. Launch state: org/repo/DNS/landing LIVE; X handle secured (profile fill pending); remaining gates = W5/W6 rebuild+gate+approval, clean-machine plugin smoke (DISPATCH-B), Show HN Tue 2026-07-14.
