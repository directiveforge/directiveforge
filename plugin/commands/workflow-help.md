---
description: Answer three questions from repo evidence — where am I, what's next, and which command/skill/agent applies. Read-only; every recommendation cites its evidence.
---

# Workflow Help

Orient anyone (human or agent) on this project's workflow: locate the current state, derive the ranked next actions, and map each to a discovered tool. READ-ONLY — this command never modifies a file.

## When to Run

- Session start, especially returning after a gap
- Unsure what to do next or which workflow asset fits the task
- Onboarding anyone onto this project's workflow
...
2. **Derive next actions** (ranked, max 3): HANDOFF "In Progress" → continue it; HANDOFF "Blocked" → surface the owner questions; DECISIONS follow-ups / reversal triggers that have fired; nothing found → recommend `/status` to re-baseline.
3. **Map each action to a discovered tool**: decision-shaped → the decision-skill router; review-shaped → reviewer agent; verification → verifier agent; context drift → `/update-context`.
...
- If no workflow files exist at all, say so.
