...
5. `git log --oneline -10` and `git status --short` (if the repo is under version control)
...
2. **Derive next actions** (ranked, max 3): HANDOFF "In Progress" → continue it; HANDOFF "Blocked" → surface owner questions; DECISIONS follow-ups / fired reversal triggers; nothing found → recommend `/status`
3. **Map each action to a discovered tool**: decision-shaped → `.claude/rules/decision-skills.md` router; review-shaped → reviewer agent; verification → verifier agent; context drift → `/update-context`
...
```
## Where you are
Branch [x] @ [hash, age] — [uncommitted: N files / clean / no VCS]
