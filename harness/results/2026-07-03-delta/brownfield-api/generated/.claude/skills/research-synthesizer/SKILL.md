...
2. `<RUN_WORKDIR_2>/l2-delta-runs/brownfield-api/.claude/rules/research.md` — the project's research-methodology rule (3-layer pointer + locked overrides)
3. `<RUN_WORKDIR_2>/l2-delta-runs/brownfield-api/docs/reference/research-synthesis-protocols.md` — Layer 2 long-form (only if the project has installed it; otherwise rely on KB-04 §3.3)
4. A Layer 3 exemplar — the canonical synthesis deliverable the project considers untouched (e.g., `<RUN_WORKDIR_2>/l2-delta-runs/brownfield-api/docs/research/slotharbor/1-decision-framework-synthesis.md`)
5. `<RUN_WORKDIR_2>/l2-delta-runs/brownfield-api/DECISIONS.md` — the entry template + most recent ledger numbering
6. Each input dossier — open and confirm Layer 1 8-gate verification stamp before this skill runs
...
Projects layer their own additional disciplines (brand filter, locale rules, compliance posture, blackout dates) on top of these nine. Configure project-specific overrides in `<RUN_WORKDIR_2>/l2-delta-runs/brownfield-api/.claude/rules/research.md`.
...
Every new DECISIONS entry from this skill's lock-skeleton output starts with the TL;DR + Master Synthesis 400w prefix, per KB-04 §2.2:
...
[Existing dense lock structure — chosen path, ranked matrix, sensitivity, staged roadmap, reversal triggers, re-verification, cross-reconciliation]
```

Backport to historic entries is not required; forward entries inherit the full template.
...
- [ ] DECISIONS #N lock skeleton uses TL;DR + Master Synthesis 400w prefix
...
- Deliverable: `<RUN_WORKDIR_2>/l2-delta-runs/brownfield-api/docs/research/slotharbor/1-decision-framework-synthesis.md`
- DECISIONS lock skeleton: drafted parallel to the deliverable. Save inline at the end of the deliverable body, or as a separate handoff file for the next session to append to `<RUN_WORKDIR_2>/l2-delta-runs/brownfield-api/DECISIONS.md` after numbering verification (`grep -c "^## Implementation #" DECISIONS.md` + 1).
...
- `<KIT_ROOT>/templates/cursor/skills/research-prompt-writer.SKILL.md.template` — Layer 1 sibling skill (8-gate input filter)
- `<RUN_WORKDIR_2>/l2-delta-runs/brownfield-api/.claude/rules/research.md` — project's 3-layer methodology rule
- `<RUN_WORKDIR_2>/l2-delta-runs/brownfield-api/docs/reference/research-synthesis-protocols.md` — Layer 2 long-form reference, installed by the kit when the project warrants it
