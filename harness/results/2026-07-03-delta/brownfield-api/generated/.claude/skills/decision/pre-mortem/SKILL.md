...
This project is Intermediate tier: `council-3-voice` and `cost-of-inaction` are NOT installed. Rule: at this tier, on a Type-1 (irreversible) verdict run `steelman` + `pre-mortem` sequentially and recommend a DECISIONS entry instead of `council-3-voice`; for 'should I do X eventually' framing, weigh the cost of waiting inline instead of routing to `cost-of-inaction`.
