...
This project is Intermediate tier: `council-3-voice` and `cost-of-inaction` are NOT installed. Rule: at this tier, if a 'do nothing' branch dominates, weigh the cost of inaction inline rather than routing to `cost-of-inaction`; if ≥3 mutually-exclusive paths surface, run `reversibility-check` then `steelman` + `pre-mortem` instead of `council-3-voice`.
