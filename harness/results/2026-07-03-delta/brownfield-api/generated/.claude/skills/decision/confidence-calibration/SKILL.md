...
This project is Intermediate tier: `bayesian-update` is NOT installed. Rule: at this tier, when new evidence should shift a prior, do the Bayesian update inline (state prior, state likelihood of the new evidence, state posterior) instead of routing to `bayesian-update`.
