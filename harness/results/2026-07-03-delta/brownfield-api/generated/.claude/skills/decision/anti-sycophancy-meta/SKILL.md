...
This project is Intermediate tier: `council-3-voice` is NOT installed. Rule: at this tier, when the user is weighing multiple options, hand off to `reversibility-check`; on a Type-1 verdict run `steelman` + `pre-mortem` sequentially and recommend a DECISIONS entry, instead of `council-3-voice`.
