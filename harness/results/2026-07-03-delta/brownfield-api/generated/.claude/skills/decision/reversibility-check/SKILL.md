...
This project is Intermediate tier: `council-3-voice` is NOT installed (the skill body already documents this fallback at 'For Type-1'). Rule: at this tier, a Type-1 verdict runs `steelman` + `pre-mortem` sequentially and recommends a DECISIONS entry, instead of `council-3-voice`.
