...
This project is Intermediate tier: `council-3-voice` is NOT installed. Rule: at this tier, when ≥3 mutually-exclusive options are on the table, run `reversibility-check` then `steelman` + `pre-mortem` sequentially and recommend a DECISIONS entry, instead of routing to `council-3-voice`.
