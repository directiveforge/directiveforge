...
> **Tier note (this project = Intermediate).** `council-3-voice` is NOT installed here (Advanced-only). Wherever this skill routes a Type-1 verdict to the council: at this tier, run `steelman` + `pre-mortem` sequentially instead and recommend a `governance/DECISIONS.md` entry recording the verdict (the Procedure step 2 fallback below already states this). A Type-2 verdict still routes to a single-pass installed skill (`inversion` / `pre-mortem` / `disconfirming-evidence-first`).
