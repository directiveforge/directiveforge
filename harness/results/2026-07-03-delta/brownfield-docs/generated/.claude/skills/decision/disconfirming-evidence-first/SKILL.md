...
> **Tier note (this project = Intermediate).** `council-3-voice` is NOT installed here (Advanced-only). Where this skill escalates ≥3-option / Type-1 decisions via `reversibility-check` to `council-3-voice`: `reversibility-check` (installed) handles the gate, and at this tier a Type-1 verdict runs `steelman` + `pre-mortem` sequentially with a `governance/DECISIONS.md` entry — no council.
