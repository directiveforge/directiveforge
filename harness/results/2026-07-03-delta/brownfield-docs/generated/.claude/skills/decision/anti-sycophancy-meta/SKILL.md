...
> **Tier note (this project = Intermediate).** `council-3-voice` is NOT installed here (Advanced-only). This skill routes to single installed skills (`pre-mortem` / `steelman` / `disconfirming-evidence-first`) by default; where it hands multi-option weighing to `reversibility-check` "which then gates whether council fires," the gate at this tier resolves a Type-1 verdict into `steelman` + `pre-mortem` sequentially + a `governance/DECISIONS.md` entry — council never fires.
