...
2. `HANDOFF.md` — in-progress / blocked items (if present)
3. `DECISIONS.md` — last 3 entries
4. `.ai-kit-manifest.json` — installed packs, presets, kit version
5. `git log --oneline -10` and `git status --short` (once under git)
...
2. **Derive next actions** (ranked, max 3): HANDOFF "In Progress" → continue; HANDOFF "Blocked" → surface owner questions; DECISIONS follow-ups; nothing found → recommend `/status`
