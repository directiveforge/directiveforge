...
3-layer architecture for research-backed decisions. Each layer addresses a different epistemic concern; none replaces another. Long-form Layer-2 reference is NOT installed for this project — rely on KB-04 §3.3.
...
3. **Stack fit** — every recommendation is implementable in SlotHarbor's actual stack (FastAPI 0.115 + SQLAlchemy 2.0 sync + Postgres/SQLite + Alembic), not "in general".
4. **Cost-benefit** — every cost claim covers launch AND 10× scale. Hidden costs (monitoring, migration ops) surfaced explicitly.
5. **Implementation** — every technique is implementable as a concrete artifact (rule, snippet, migration, prompt). Abstract advice fails.
6. **Cross-reference** — every load-bearing claim corroborated by ≥2 sources OR flagged as single-source-uncorroborated.
7. **Existing knowledge** — every claim checked against existing project docs + DECISIONS.md. If answered, point to the existing doc; don't re-derive.
...
1. **Severity tag** per finding — BLOCKER / HIGH / MEDIUM / LOW. Cross-cutting calibration.
...
1. **Intake** — per-dossier core claim (≤20 words), severity, cluster grouping; preview-flag contradictions for Protocol 2.
...
5. **Methodology Audit** — neutral framing of methodological diversity (highest-uncertainty, NEVER "weakest").
6. **Master Synthesis (≤400w)** — Established Consensus / Active Debates / Strongest Evidence / Key Open Question.
7. **Assumption Catalogue** — foundational unstated assumptions: risk if false × consequence tier × monitoring approach.
8. **"So What" TL;DR** — 3 numbered points for a smart non-expert; links to the full Layer 3 body.

## LAYER 3 — FORWARD CONSTRUCTION

Reads the Layer 1 corpus + Layer 2 outputs; produces a deliverable that (1) ranks options with twin-view when an assumption splits the answer, (2) runs sensitivity analysis under multiple weight regimes + scale shocks, (3) builds a staged roadmap with trigger conditions per stage, (4) pre-loads Phase 3 elicitation, (5) drafts the next architect-prompt skeleton, (6) records a re-verification schedule, (7) writes decidable reversal triggers per stage, (8) reconciles against prior DECISIONS entries. §1–§8 are the Layer 2 outputs verbatim; §9+ are net-new.

## UNIVERSAL BLOCKS A / B / C (mandatory per dossier)

- **Block A — Context Envelope:** SlotHarbor's binding constraints (small maintenance-mode service, sync stack locked per DECISIONS #1, Postgres prod / SQLite dev, no test tooling). 8–15 lines.
