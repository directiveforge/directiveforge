...
description: Review a finished artifact (email, analysis, plan, code change, claim) by searching ONLY for evidence the claim is wrong — not by balancing pros and cons. Use on triggers like "is my email good?", "is this analysis right?", "review this", "quick review", "is my tone right". This skill is asymmetric on purpose; balanced review is the sycophantic default and produces validation rather than audit.
severity: HIGH
confidence: 0.80
surface: Chat
---
...
> **Trigger**: User asks for review of a finished or near-finished artifact — "is my email good?", "is this analysis right?", "review this", "quick review", "is my tone right?", "does this work?". The artifact already exists; the user wants an audit, not a brainstorm. This is the single-pass lightweight skill that runs on Chat surface; for argument-shaped positions use `steelman`, for plans use `pre-mortem`.
