...
> **Phase 4 architect prompt** that turns a locked [DECISIONS.md]({{DECISIONS_PATH}}) entry into actionable engineering steps. The 14 sections + hard ceilings below are load-bearing — see [KB-04 §5]({{AI_KIT_PATH}}/knowledge-base/KB-04-DECISION-ENGINEERING.md#5-pillar-4--the-14-section-architect-prompt) for why each one exists.
<!-- {{DECISIONS_PATH}}: the project's ledger relative to this file, e.g. "../../DECISIONS.md" or "../../governance/DECISIONS.md". {{AI_KIT_PATH}}: absolute path to the AI Workflow Engineering Kit, e.g. "/Users/jane/Projects/AI" — relative ../../../ escapes any project that isn't the kit itself. -->
>
...
Apply the [8-gate framework from KB-04 §3.1]({{AI_KIT_PATH}}/knowledge-base/KB-04-DECISION-ENGINEERING.md#31-layer-1--the-8-gate-input-filter) to every load-bearing claim in every deliverable:
...
For full depth on each section's purpose and failure modes, read [KB-04 §5]({{AI_KIT_PATH}}/knowledge-base/KB-04-DECISION-ENGINEERING.md#5-pillar-4--the-14-section-architect-prompt). For a worked example at production rigor, see [the private lab's worked instance (not shipped)]({{AI_KIT_PATH}}/the private lab's worked instance (not shipped)).
