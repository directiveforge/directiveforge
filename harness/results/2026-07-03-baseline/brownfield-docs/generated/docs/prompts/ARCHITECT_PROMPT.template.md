...
> **Architect prompt** that turns a locked [`governance/DECISIONS.md`](../../governance/DECISIONS.md) `D{N}` entry into actionable steps. This is a docs/ops repo — "engineering steps" means new playbooks, registry proposals, and ledger entries, never code. The 14 sections + hard ceilings below are load-bearing — see [KB-04 §5](<KIT_ROOT>/knowledge-base/KB-04-DECISION-ENGINEERING.md#5-pillar-4--the-14-section-architect-prompt) for why each one exists.
<!-- This repo's ledger is governance/DECISIONS.md with the house D{N} heading format (NOT "Implementation #N"). Instantiate this template as docs/prompts/IMPL_NN_<topic>.md when locking a research-backed decision (supplier, lease, expansion). -->
>
...
For full depth on each section's purpose and failure modes, read [KB-04 §5](<KIT_ROOT>/knowledge-base/KB-04-DECISION-ENGINEERING.md#5-pillar-4--the-14-section-architect-prompt). Worked examples ship in the kit's `case-studies/` directory.
