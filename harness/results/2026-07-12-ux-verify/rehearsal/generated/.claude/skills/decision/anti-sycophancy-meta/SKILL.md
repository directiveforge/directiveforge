...
description: FIRST RESPONDER on any generic ownership-evaluation ask — the user brings their own idea, plan, or artifact and asks for a verdict in neutral terms ("is it any good?", "give me your take", "thoughts on my plan?", "should I do this?", "is my idea worth it?"). Detects the ownership signal, refuses to validate before re-framing, then routes to pre-mortem OR steelman OR disconfirming-evidence-first by artifact type. Pre-empts its own downstream siblings on the generic utterance. Deferral clause — does NOT fire when the user names a technique or requests a specific analytical frame (a pre-mortem, a steelman, an inversion into constraints, a flaws-only teardown, an explicit "how could this fail"); those go straight to the named technique. Sycophancy runs a 58.19% baseline across frontier models per SycEval; the fix is to invert the framing order before any evaluation lands.
severity: BLOCKER
confidence: 0.95
surface: All
---
...
> **Trigger**: First responder on any user message containing an ownership signal ("my plan", "my idea", "my X") combined with a generic evaluation request ("what do you think", "is it any good", "review this", "give me your take", "thoughts on my plan"). Also fires on bare "is this a good idea?" / "should I do X?" framings — even without an explicit possessive — because the implicit ownership is present. **Deferral clause**: does NOT fire when the user names a specific technique or requests a specific analytical frame (a pre-mortem, a steelman, "how could this fail?", "flip it into constraints", "tell me only what's wrong") — those route straight to the named technique. Does not fire on emotional-support framings (see §"When NOT to auto-fire").
...
1. `<KIT_ROOT>/knowledge-base/KB-05-CONVERSATIONAL-DECISION-ENGINEERING.md` §4 (this skill is the canonical anti-sycophancy meta-skill spec — §4.1 detection, §4.2 why-first, §4.3 opt-out).
2. `<KIT_ROOT>/research/2026-05-26-decision-techniques.md` §2 row 12 + §4 (the four anti-sycophancy patterns) + §7 quote #4 (Anthropic's verbatim definition).
...
1. **Detect the ownership signal.** Scan the user message for generic ownership-evaluation patterns:
   ...
   - **Deferral check**: if the user has named a specific technique or requested a specific analytical frame (a pre-mortem, a steelman, "how could this fail?", "flip it into constraints / turn the risks into requirements", "tell me only what's wrong / tear into it"), do NOT fire — hand the turn directly to the named technique. This skill owns the *generic* utterance; the siblings own their frame-specific requests.
...
- KB-05: `<KIT_ROOT>/knowledge-base/KB-05-CONVERSATIONAL-DECISION-ENGINEERING.md` §4 (anti-sycophancy as meta-skill), §7 (four anti-sycophancy patterns), §10 (failure modes — contrarianism-cascade row), §13 (anti-pattern #1: "Skip anti-sycophancy-meta, evaluate first").
- Dossier: `<KIT_ROOT>/research/2026-05-26-decision-techniques.md` §2 row 12, §4 (four patterns), §7 quotes #4, #5, #6, #7, #8.
- Primary sources:
...
This project installs the **Starter** decision tier — 5 BLOCKER skills only: `anti-sycophancy-meta`, `pre-mortem`, `steelman`, `confidence-calibration`, `reversibility-check`. Siblings NOT installed at this tier (`disconfirming-evidence-first`, `council-3-voice`) resolve to inline fallbacks:

- **Finished artifact (email / draft / code / copy)** → normally `disconfirming-evidence-first` (not installed); run that pass INLINE — list what's wrong ranked by severity, then force a verdict ("if I had only this evidence, would I ship?").
- **Multiple options / Type-1 escalation** → `reversibility-check` (installed) gates it; `council-3-voice` is not installed, so run `steelman` + `pre-mortem` sequentially and recommend a DECISIONS.md entry.
