...
1. `<KIT_ROOT>/knowledge-base/KB-05-CONVERSATIONAL-DECISION-ENGINEERING.md` §5.4 (council invocation threshold — this skill IS the gating mechanism), §6 (council reference architecture), §6.5 (cost gate).
2. `<KIT_ROOT>/research/2026-05-26-decision-techniques.md` §2 row 8 (canonical procedure + output column), §3 (council architecture).
...
- KB-05: `<KIT_ROOT>/knowledge-base/KB-05-CONVERSATIONAL-DECISION-ENGINEERING.md` §5.4 (gating role), §6 (council architecture)
- Dossier: `<KIT_ROOT>/research/2026-05-26-decision-techniques.md` §2 row 8
- Primary sources:
...
Not installed at this tier (Starter): `council-3-voice`, `inversion`, `disconfirming-evidence-first`. Fallbacks:
- **Type-1 verdict** → the skill body already handles this: run `steelman` + `pre-mortem` sequentially (no council) and recommend a DECISIONS.md entry.
- **Type-2, "is this a good idea?"** → normally `inversion`; here route to `anti-sycophancy-meta` → `steelman` (argument) or `pre-mortem` (plan), or do the flip-into-constraints move INLINE.
- **Type-2, finished artifact** → run the single-pass disconfirming review INLINE instead of `disconfirming-evidence-first`.
