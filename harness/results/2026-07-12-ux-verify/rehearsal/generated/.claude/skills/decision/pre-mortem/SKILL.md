...
1. `<KIT_ROOT>/knowledge-base/KB-05-CONVERSATIONAL-DECISION-ENGINEERING.md` §4 (anti-sycophancy as meta-skill) and §7.1 (frame-reversal pattern).
2. `<KIT_ROOT>/research/2026-05-26-decision-techniques.md` §2 row 1 (the canonical procedure) and §7 quote #4 (Anthropic's sycophancy definition).
...
4. If the user is on a Type-1 (irreversible) decision per `.claude/skills/decision/reversibility-check/SKILL.md`, escalate to `council-3-voice` after this skill completes — pre-mortem alone is single-perspective.
...
- KB-05: `<KIT_ROOT>/knowledge-base/KB-05-CONVERSATIONAL-DECISION-ENGINEERING.md` §4, §7.1, §10.
- Dossier: `<KIT_ROOT>/research/2026-05-26-decision-techniques.md` §2 row 1.
- Primary sources:
...
Not installed at this tier (Starter): `council-3-voice`, `second-order-thinking`, `disconfirming-evidence-first`, `cost-of-inaction`, `10-10-10`. Fallbacks:
- **Type-1 (irreversible) follow-up** → no council here; run `steelman`, then re-run this pre-mortem focused on the top-ranked risk, and recommend a DECISIONS.md entry.
- **Downstream-effects follow-up** → analyze the second/third-order chain INLINE (name the knock-on effects) instead of handing off to `second-order-thinking`.
- **Tone checks / single-fact lookups** (the "When NOT to use" route) → run the single-pass disconfirming review INLINE instead of routing to `disconfirming-evidence-first`.
- **"Should I do X eventually" timing questions** → answer the cost-of-waiting INLINE (name what decays while waiting) instead of routing to `cost-of-inaction` or `10-10-10`.
