...
- **Documented-invocation fix (PH-7, public commit #3):** the quickstart advertised `/report-friction`, but under a plugin install the working form is `/directiveforge:report-friction`; README + QUICK_START + KIT-INSTALL-PIPELINE corrected (invocations namespaced, false "skills-only namespacing" claim removed). Caught by the clean-machine install smoke on CLI 2.1.206.
