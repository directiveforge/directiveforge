...
*Compiled: 2026-04-21 · Last refreshed: 2026-06-26 (v0.16.4 §1a Fable/Mythos SUSPENDED + new §1b model-availability resilience; v0.16.0 §3a/§4a) · Refresh mechanism: vigilance cadence (daily pulse → monthly integration), NOT a calendar date — a dated marker with no firing mechanism silently expires (field-proven 2026-06-10) · Source: `CLAUDE-SURFACE-MATRIX.md` (April 2026 dossier)*
...
- **Claude Code** — terminal-first agentic coding tool, also on IDE extensions / Desktop "Code" tab / web / iOS. **Killer use case: deep repo work** — Plan mode, checkpoints/`/rewind`, hooks, worktrees, custom subagents, effort control (the `/effort` slider was relabeled **Faster/Smarter** in v2.1.154), Skills, 1M-token context auto-on Max/Team/Enterprise for Opus 4.6/4.7/4.8 + Sonnet 4.6. **Opus 4.8 is the default model as of Claude Code v2.1.154 (2026-05-29); Fable 5 became selectable in v2.1.170 (2026-06-09) but is ⛔ SUSPENDED since 2026-06-12 — see §1a.** No Research mode, no Artifacts, no native Office export. [dossier §3.3]

---

## 1a. Model-class routing (Mythos tier) — ⛔ SUSPENDED since 2026-06-12

**Architectural fact (2026-06-12, 5:21pm ET → present):** Claude **Fable 5 + Mythos 5 are administratively unavailable** for all customers under a US-government export-control directive ([anthropic.com/news/fable-mythos-access](https://www.anthropic.com/news/fable-mythos-access) · [Fortune 2026-06-13](https://fortune.com/2026/06/13/anthropic-disables-fable-mythos-export-controls-national-security-threat/)). Still offline as of 2026-06-26, **no restoration date**. **Do NOT author routing that depends on these models** — Claude Code Fable selections error-redirect to Opus 4.8. Restoration is an **event to watch, not a near-term plan**; the general resilience doctrine this episode taught is §1b.

**The spec (reference only — applies if/when restored):** Mythos-class is a tier *above* Opus (shipped 2026-06-09). **Fable 5** (`claude-fable-5`) was the first GA member — SOTA on long/complex tasks, $10/M in + $50/M out (≈2× Opus), 1M context, 128K max output, Jan-2026 cutoff; cyber/bio/distillation requests fell back to Opus 4.8 — **made *visible* after Anthropic's 2026-06-11 apology** for the original silent behavior. **Mythos 5** (`claude-mythos-5`) = same weights, safeguards lifted, restricted-access only. While live, the route was: Fable for the longest-horizon/hardest work where the ≈2× price is justified; Opus 4.8 the cost-rational default.

**Compliance carry-forward (applies even while suspended):** Mythos-class traffic carries **mandatory 30-day retention with no ZDR opt-out** — a procurement-review trigger for regulated / zero-retention workloads (Microsoft restricted internal use over exactly this; see KB-02 §8.5).

---

## 1b. Model-availability resilience (the Fable 5 lesson, 2026-06-12)

A named model can become **administratively unavailable mid-deployment** — not slow, not degraded, *gone* — in hours. Fable 5 is the type specimen (export-control recall); ordinary model deprecations are the slower cousin. Design for it:
...
11. **Do not route to Fable 5 / Mythos 5 — they are ⛔ SUSPENDED (2026-06-12, §1a)** and administratively unavailable; any rubric that sends work to them fails today. Even if restored, Fable is a distinct class (Mythos tier) with different economics (≈2× output price, mandatory 30-day retention), and its classifier fallback to Opus 4.8 is now *visible* (post-2026-06-11 apology), not silent — so "newest Opus = best default" rubrics still need an explicit class gate. The deeper lesson — never pin to one model ID — is §1b. [§1a, §1b, anthropic.com/news/fable-mythos-access]
...
- **Off-cycle triggers:** new Mythos-class (Fable/Mythos) or Opus/Sonnet/Haiku model; new Desktop tab/surface; beta→GA on Computer Use / Claude in Chrome / Claude Code on the web; any plan-tier pricing or included-features change — **watch for restoration of the suspended Fable 5 / Mythos 5 (§1a) — no date as of 2026-06-26**; any Managed Agents primitive reaching GA or Routines leaving research preview (re-grade §3a status caveats; KB-06 §10 carries the full trigger list). [§9]
